...
DROP VIEW IF EXISTS s5_4; CREATE OR REPLACE VIEW s5_4 AS SELECT 'S5.4 heeft nog geen uitwerking - misschien moet je de DROP VIEW ... regel nog activeren?' AS resultaat;
...
DROP VIEW IF EXISTS s5_4_test; CREATE OR REPLACE VIEW s5_4_test AS
SELECT * FROM (VALUES
    ('JANSEN'::VARCHAR(12)),
    ('CLERCKX'),
    ('SCHOTTEN'),
